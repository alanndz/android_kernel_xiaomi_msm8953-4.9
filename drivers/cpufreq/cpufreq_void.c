/*
 * drivers/cpufreq/cpufreq_void.c
 *
...
#include <trace/events/cpufreq_void.h>

static DEFINE_PER_CPU(struct update_util_data, update_util);

struct cpufreq_void_policyinfo {
	...
	struct cpufreq_void_tunables *cached_tunables;
	struct sched_load *sl;
};

/* Protected by per-policy load_lock */
struct cpufreq_void_cpuinfo {
	u64 time_in_idle;
...
static DEFINE_PER_CPU(struct cpufreq_void_policyinfo *, polinfo);
static DEFINE_PER_CPU(struct cpufreq_void_cpuinfo, cpuinfo);
...
static struct cpufreq_void_tunables *common_tunables;
static struct cpufreq_void_tunables *cached_common_tunables;

static struct attribute_group *get_sysfs_attr(void);

/* Round to starting jiffy of next evaluation window */
static u64 round_to_nw_start(u64 jif,
			     struct cpufreq_void_tunables *tunables)
{
...
static void cpufreq_void_timer_resched(unsigned long cpu,
					      bool slack_only)
{
...
{
	struct cpufreq_void_policyinfo *ppol;
	unsigned long flags;
 ...
 */
static void cpufreq_void_timer_start(
	struct cpufreq_void_tunables *tunables, int cpu)
{
	struct cpufreq_void_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_void_cpuinfo *pcpu;
...
static unsigned int freq_to_targetload(
	struct cpufreq_void_tunables *tunables, unsigned int freq)
{
	int i;
	unsigned int ret;
	unsigned long flags;
 ...
 */
static unsigned int choose_freq(struct cpufreq_void_policyinfo *pcpu,
		unsigned int loadadjfreq)
{
...
{
	struct cpufreq_void_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_void_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
	struct cpufreq_void_tunables *tunables =
		ppol->policy->governor_data;
...
static unsigned int sl_busy_to_laf(struct cpufreq_void_policyinfo *ppol,
				   unsigned long busy)
{
	int prev_load;
	struct cpufreq_void_tunables *tunables =
...
static void cpufreq_void_timer(int data)
{
	s64 now;
	...
	int cpu_load;
	int pol_load = 0;
	struct cpufreq_void_policyinfo *ppol = per_cpu(polinfo, data);
	struct cpufreq_void_tunables *tunables =
		ppol->policy->governor_data;
	...
	for_each_cpu(cpu, ppol->policy->cpus) {
		...
		trace_cpufreq_void_cpuload(cpu, cpu_load, new_load_pct,
						  prev_l, pred_l);
	...
	trace_cpufreq_void_target(max_cpu, pol_load, ppol->target_freq,
					 ppol->policy->cur, new_freq);
...
rearm:
	cpufreq_void_timer_resched(data, false);
...
static int cpufreq_void_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			if (ppol->target_freq != ppol->policy->cur)
				__cpufreq_driver_target(ppol->policy,
							ppol->target_freq,
							CPUFREQ_RELATION_H);
			trace_cpufreq_void_setspeed(cpu,
						     ppol->target_freq,
						     ppol->policy->cur);
			up_read(&ppol->enable_sem);
		}
...
static void cpufreq_void_boost(struct cpufreq_void_tunables *tunables)
{
	int i;
	int anyboost = 0;
...
static enum hrtimer_restart cpufreq_void_hrtimer(struct hrtimer *timer)
{
	struct cpufreq_void_policyinfo *ppol = container_of(timer,
			struct cpufreq_void_policyinfo, notif_timer);
	int cpu;
	...
	cpu = ppol->notif_cpu;
	trace_cpufreq_void_load_change(cpu);
	del_timer(&ppol->policy_slack_timer);
	cpufreq_void_timer(cpu);
...
static int cpufreq_void_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_void_notifier,
};
...
static ssize_t show_hispeed_freq(struct cpufreq_void_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->hispeed_freq);
}

static ssize_t store_hispeed_freq(struct cpufreq_void_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_go_hispeed_load(struct cpufreq_void_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->go_hispeed_load);
}

static ssize_t store_go_hispeed_load(struct cpufreq_void_tunables
...
static ssize_t store_min_sample_time(struct cpufreq_void_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_rate(struct cpufreq_void_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->timer_rate);
}

static ssize_t store_timer_rate(struct cpufreq_void_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val, val_round;
	struct cpufreq_void_tunables *t;
	int cpu;
...
static ssize_t show_timer_slack(struct cpufreq_void_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->timer_slack_val);
}

static ssize_t store_timer_slack(struct cpufreq_void_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_boost(struct cpufreq_void_tunables *tunables,
			  char *buf)
{
	return sprintf(buf, "%d\n", tunables->boost_val);
}

static ssize_t store_boost(struct cpufreq_void_tunables *tunables,
			   const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (tunables->boost_val) {
		trace_cpufreq_void_boost("on");
		if (!tunables->boosted)
			cpufreq_void_boost(tunables);
	} else {
		tunables->boostpulse_endtime = ktime_to_us(ktime_get());
		trace_cpufreq_void_unboost("off");
	}

	return count;
}

static ssize_t store_boostpulse(struct cpufreq_void_tunables *tunables,
				const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	trace_cpufreq_void_boost("pulse");
	if (!tunables->boosted)
		cpufreq_void_boost(tunables);
	return count;
}

static ssize_t show_boostpulse_duration(struct cpufreq_void_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%d\n", tunables->boostpulse_duration_val);
}

static ssize_t store_boostpulse_duration(struct cpufreq_void_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_io_is_busy(struct cpufreq_void_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->io_is_busy);
}

static ssize_t store_io_is_busy(struct cpufreq_void_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static int cpufreq_void_enable_sched_input(
			struct cpufreq_void_tunables *tunables)
{
	int rc = 0, j;
	struct cpufreq_void_tunables *t;
...
static int cpufreq_void_disable_sched_input(
			struct cpufreq_void_tunables *tunables)
{
	mutex_lock(&sched_lock);
			...
			const char *buf, size_t count)
{
	...
	if (val)
		ret = cpufreq_void_enable_sched_input(tunables);
	else
		ret = cpufreq_void_disable_sched_input(tunables);
...
static struct attribute *void_attributes_gov_sys[] = {
...
static struct attribute_group void_attr_group_gov_sys = {
	.attrs = void_attributes_gov_sys,
	.name = "void",
};

/* Per policy governor instance */
static struct attribute *void_attributes_gov_pol[] = {
...
static struct attribute_group void_attr_group_gov_pol = {
	.attrs = void_attributes_gov_pol,
	.name = "void",
};
...
static struct attribute_group *get_sysfs_attr(void)
{
	if (have_governor_per_policy())
		return &void_attr_group_gov_pol;
	else
		return &void_attr_group_gov_sys;
}

static void cpufreq_void_nop_timer(unsigned long data)
{
}

static struct cpufreq_void_tunables *alloc_tunable(
					struct cpufreq_policy *policy)
{
	struct cpufreq_void_tunables *tunables;
...
static void irq_work(struct irq_work *irq_work)
{
	...
	ppol = container_of(irq_work, struct cpufreq_void_policyinfo,
			    irq_work);

	cpufreq_void_timer(smp_processor_id());
	spin_lock_irqsave(&ppol->irq_work_lock, flags);
	ppol->work_in_progress = false;
	spin_unlock_irqrestore(&ppol->irq_work_lock, flags);
}

static struct cpufreq_void_policyinfo *get_policyinfo(
					struct cpufreq_policy *policy)
{
	struct cpufreq_void_policyinfo *ppol =
	...
	ppol->policy_slack_timer.function = cpufreq_void_nop_timer;
	hrtimer_init(&ppol->notif_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	ppol->notif_timer.function = cpufreq_void_hrtimer;
...
static struct cpufreq_void_tunables *get_tunables(
				struct cpufreq_void_policyinfo *ppol)
{
	if (have_governor_per_policy())
...
/* Void Governor callbacks */
struct void_governor {
	struct cpufreq_governor gov;
	unsigned int usage_count;
};

static struct void_governor void_gov;

#define CPU_FREQ_GOV_VOID	(&void_gov.gov)

int cpufreq_void_init(struct cpufreq_policy *policy)
{
	int rc;
	struct cpufreq_void_policyinfo *ppol;
	struct cpufreq_void_tunables *tunables;
	...
	if (!void_gov.usage_count++)
		cpufreq_register_notifier(&cpufreq_notifier_block,
				CPUFREQ_TRANSITION_NOTIFIER);

	if (tunables->use_sched_load)
		cpufreq_void_enable_sched_input(tunables);
...
void cpufreq_void_exit(struct cpufreq_policy *policy)
{
	struct cpufreq_void_tunables *tunables;
	...
	if (tunables->use_sched_load)
		cpufreq_void_disable_sched_input(tunables);
}

int cpufreq_void_start(struct cpufreq_policy *policy)
{
	struct cpufreq_void_policyinfo *ppol;
	struct cpufreq_frequency_table *freq_table;
	struct cpufreq_void_tunables *tunables;
	...
	ppol->last_evaluated_jiffy = get_jiffies_64();
	cpufreq_void_timer_start(tunables, policy->cpu);
...
void cpufreq_void_stop(struct cpufreq_policy *policy)
{
	struct cpufreq_void_policyinfo *ppol;
	struct cpufreq_void_tunables *tunables;
...
void cpufreq_void_limits(struct cpufreq_policy *policy)
{
	struct cpufreq_void_policyinfo *ppol;
	struct cpufreq_void_tunables *tunables;
	...
	if (ppol->governor_enabled) {
		if (policy->min < ppol->min_freq)
			cpufreq_void_timer_resched(policy->cpu,
							  true);
		ppol->min_freq = policy->min;
	}
	up_read(&ppol->enable_sem);
...
static struct void_governor void_gov = {
	.gov = {
		...
		.owner			= THIS_MODULE,
		.init			= cpufreq_void_init,
		.exit			= cpufreq_void_exit,
		.start			= cpufreq_void_start,
		.stop			= cpufreq_void_stop,
		.limits			= cpufreq_void_limits,
	}
};

static int __init cpufreq_void_gov_init(void)
{
	...
	speedchange_task =
		kthread_create(cpufreq_void_speedchange_task, NULL,
			       "cfvoid");
	...
	return cpufreq_register_governor(CPU_FREQ_GOV_VOID);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_VOID
struct cpufreq_governor *cpufreq_default_governor(void)
{
	return CPU_FREQ_GOV_VOID;
}

fs_initcall(cpufreq_void_gov_init);
#else
module_init(cpufreq_void_gov_init);
#endif

static void __exit cpufreq_void_gov_exit(void)
{
	int cpu;

	cpufreq_unregister_governor(CPU_FREQ_GOV_VOID);
...
module_exit(cpufreq_void_gov_exit);
